...
def _github_create_issue_sync(*, github_token: str, title: str, body: str) -> CreatedIssue:
    ...
    return CreatedIssue(number=int(data["number"]), url=str(data["html_url"]), title=str(data.get("title") or title))


async def github_create_issue(*, github_token: str, title: str, body: str) -> CreatedIssue:
    return await asyncio.to_thread(_github_create_issue_sync, github_token=github_token, title=title, body=body)
...
async def issue_command(ctx: commands.Context, *, content: str = "") -> None:
    ...
    body = _build_issue_body(content=content, author=ctx.author, channel=ctx.channel, message_url=message_url)
    try:
        created = await github_create_issue(github_token=github_token, title=title, body=body)
