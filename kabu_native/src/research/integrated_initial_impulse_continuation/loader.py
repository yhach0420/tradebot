"""PUSH loader — stride=1 only, canonical Buy1/Sell1 semantics."""
...
from research.integrated_initial_impulse_continuation.constants import CAPTURE_ROOT, LOT, STRIDE
...
def iter_day_ticks(day: str, *, stride: int = STRIDE) -> Iterator[Tick]:
    assert stride == 1, "IIC forbids event sampling (stride must be 1)"
        ...
        with fp.open("r", encoding="utf-8", errors="ignore") as f:
            for i, line in enumerate(f):
                ...
                yield Tick(
                    ...
                    session=sess, trade_side=side, event_seq=seq, prev_ask_qty=paq,
                )
...
def first_valid_ask(ticks: list[Tick], i: int) -> Optional[tuple[int, float]]:
    for j in range(i, min(len(ticks), i + 40)):
