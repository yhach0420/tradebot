"""Publish E1_X26 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "RegistryIntegrity", "DiscoveryDataContract", "PreviouslyVisibleData",
    "FamilyTags", "FamilyMarginScores", "FamilyRouting", "CandidateBalancedCalibration",
    "AnchorWeightedSensitivity", "CalibrationDisagreement", "ParameterGrids", "CommonControls",
    "PBV2Control", "QuickExitDesign", "PullbackExitDesign", "ContinuationExitDesign",
    "DelayedExitDesign", "SpikeExitDesign", "ExitRegistry", "ExitPriority",
    "DiscoveryTriggerReplay", "ExitReasonCoverage", "ExitManifest", "X27Handoff",
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "family_exits", "value": report.get("family_exit_count")},
        {"item": "common_controls", "value": report.get("common_control_count")},
    ]
    ...
    md = [
        ...
        f"- manifest: `{report.get('manifest_id')}` sha=`{report.get('manifest_sha256')}`",
        f"- EXIT library: common={report.get('common_control_count')} family={report.get('family_exit_count')}",
        f"- PBv2: `{(report.get('pbv2_control') or {}).get('status')}`",
        f"- X27 handoff masks: `{(report.get('x27_handoff') or {}).get('unique_masks')}`",
        f"- tests: {tests.get('passed')}/{tests.get('total')} · A/B: {det.get('ab_match')}",
        "- submit/cancel/live: 0/0/0",
        "- EXIT parameters: DISCOVERY_ONLY_MECHANICAL_CALIBRATION",
        "",
    ]
