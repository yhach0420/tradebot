"""Publish E1_X21 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "DateRoles", "FeatureRegistry", "UnavailableFeatures",
    "DuplicateGroups", "ThresholdRegistry", "CandidateRegistry", "CandidateImplementation",
    "SingleDirectional", "SinglePeriodResults", "TwoFeatureRegistry", "TwoFeatureDirectional",
    "BatchExecution", "ExitPolicy", "CanonicalExitIdentity", "CanonicalExitParity",
    "BenchmarkExitDistinctness", "ExecutionCoverage", "PairEconomics", "ExitSensitivity",
    "DailyResults", "SymbolResults", "CandidateStatus", "FamilyViews",
    "PromotionBundleProposal", "ReservedDates", "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "single_n", "value": report.get("single_entry_logic_count")},
        {"item": "two_n", "value": report.get("two_feature_entry_logic_count")},
    ...
    md = [
        ...
        f"- features available/unavailable: `{report.get('available_feature_count')}` / `{report.get('unavailable_feature_count')}`",
        f"- single / two-feature / total logics: `{report.get('single_entry_logic_count')}` / `{report.get('two_feature_entry_logic_count')}` / `{report.get('total_implemented_logic_count')}`",
        f"- benchmark EXITs: `{report.get('benchmark_exits')}`",
        f"- pairs: `{report.get('pair_count')}`",
        f"- directional / economic / sensitive / weak: `{report.get('status_counts')}`",
        f"- 20260804: `UNCLASSIFIED_DO_NOT_OPEN` · precommit: not created",
