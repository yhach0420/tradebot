"""Publish E1_X14 three artifacts only."""
...
SHEETS = [
    "Index", "SourceInventory", "PopulationBias", "DateQuality", "Schema", "FixedGrid",
    "Freshness", "FeatureContract", "RelativeUniverse", "ClusterContract", "LabelContract",
    "DateSplit", "SingleFeatureResults", "PricePath", "VolumeContinuity", "RelativeStrength",
    "DailyBalance", "SymbolBalance", "LODO", "LOSO", "RPFEOverlap", "Leakage",
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets = {name: sh.get(name) or [{"note": "empty"}] for name in SHEETS}
    sheets["Index"] = [
        {"item": "run_id", "value": report.get("run_id")},
        {"item": "verdict", "value": report.get("verdict")},
        {"item": "population", "value": (report.get("population") or {}).get("verdict")},
        {"item": "usable_start", "value": report.get("actual_usable_start")},
    ]
    ...
    md = [
        ...
        f"- population: `{(report.get('population') or {}).get('verdict')}`",
        f"- usable start: `{report.get('actual_usable_start')}` · days: {report.get('n_trading_days')}",
        f"- Price Path: `{(report.get('price_path') or {}).get('verdict')}`",
        f"- Volume Continuity: `{(report.get('volume_continuity') or {}).get('verdict')}`",
        f"- Relative Strength: `{(report.get('relative_strength') or {}).get('verdict')}`",
