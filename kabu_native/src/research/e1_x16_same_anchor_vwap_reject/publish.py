"""Publish E1_X16 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "AnchorContract", "ThresholdContract", "Variants",
    "AvailabilityControl", "DirectionalOutcomes", "RiskDistribution", "RejectEffectiveness",
    "DailyResults", "SymbolResults", "Exclude20260722", "ExcludeSymbols", "IncrementalValue",
    "CandidateSelection", "ProspectivePrecommit", "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "study_type", "value": report.get("study_type")},
        {"item": "selected", "value": report.get("selected_candidate")},
    ]
    ...
    md = [
        ...
        f"- study_type: `{report.get('study_type')}` (INTERNAL exploratory — not holdout / not robust)",
        f"- A0/A1/A2/A2_Rejected/A3/A4 support: `{report.get('supports')}`",
        f"- A1 availability effect: `{report.get('a1_availability_effect')}`",
        f"- A2 pure VWAP reject effect: `{report.get('a2_pure_vwap_reject_effect')}`",
        f"- A3 status: `{report.get('a3_status')}` · A4 status: `{report.get('a4_status')}`",
        f"- selected: `{report.get('selected_candidate')}`",
        f"- precommit: `{report.get('prospective_precommit_status')}`",
