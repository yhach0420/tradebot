"""
Phase368: Symbol reentry cluster guard shadow validation (post Phase355+364).
"""
...
from small_paper.symbol_reentry_cluster_entry_guard_shadow import (
    SPLIT_VARIANTS,
    VARIANT_LABELS,
    _float,
    _pf,
...
TRADE_FIELDS = [
    ...
    "symbol_reentry_guard_shadow_blocked",
    "symbol_reentry_shadow_pnl_yen_100",
    "symbol_reentry_shadow_delta_yen",
    "symbol_reentry_guard_variant",
    "is_low_mfe_stop",
]
...
@dataclass
class Phase368SymbolReentryClusterValidation:
    reports_dir: Path
    session_results: list[dict[str, Any]] = field(default_factory=list)

    def paths(self) -> dict[str, Path]:
        return {
            "summary": self.reports_dir / "phase368_symbol_reentry_cluster_guard_summary.json",
            "by_variant": self.reports_dir
            / "phase368_symbol_reentry_cluster_guard_by_variant.csv",
            "by_day": self.reports_dir / "phase368_symbol_reentry_cluster_guard_by_day.csv",
            "by_symbol": self.reports_dir
            / "phase368_symbol_reentry_cluster_guard_by_symbol.csv",
            "trades": self.reports_dir / "phase368_symbol_reentry_cluster_guard_trades.csv",
        }
    ...
    def _aggregate(self) -> dict[str, Any]:
        ...
        for variant in SPLIT_VARIANTS:
            ...
            for sr in self.session_results:
                ...
                sess_shadow = sum(
                    float(_float(t.get("symbol_reentry_shadow_pnl_yen_100")) or 0.0)
                ...
                by_am_pm_variant[(kind, variant)]["dynamic40_delta_yen"] += round(
                    dyn_s - dyn_a, 2
                )
            ...
            for day, day_rows in by_day.items():
                ...
                d_shadow = sum(
                    float(_float(t.get("symbol_reentry_shadow_pnl_yen_100")) or 0.0)
                    for t in day_variant_rows
                )
                d_delta = round(d_shadow - d_actual, 2)
                blocked_day = [t for t in day_variant_rows if t.get("symbol_reentry_guard_shadow_blocked")]
                dk = (day, variant)
                ...
                by_day_variant[dk]["stop_hit_reduction_count"] += sum(
                    1
                    for t in blocked_day
                    if t.get("exit_reason_canonical") == "stop_hit"
                )
        ...
        return {
            ...
            "production_shadow_continue": not production_adopt and production_shadow,
    ...
    def finalize_outputs(
        ...
        summary = {
            "phase": 368,
            "title": "symbol_reentry_cluster_guard_shadow_validation",
            "generated_at": datetime.now(JST).isoformat(timespec="seconds"),
            "population": "phase355_plus_phase364_production_kept_trades",
            "guard_rule": (
                "Shadow-block entry when same day/symbol had prior low-MFE stop_hit "
                f"(peak_mfe<{0.3}%) per variant scope"
            ),
            ...
            "conclusion": {
                ...
                "delta_pf": (
                    round((best_row.get("shadow_pf") or 0) - (best_row.get("actual_pf") or 0), 4)
                    if best_row.get("shadow_pf") is not None
                    and best_row.get("actual_pf") is not None
                    else None
                ),
                ...
                "production_shadow_continue": agg["production_shadow_continue"],
                "recommendation": (
                    f"Adopt {best} symbol reentry cluster guard in production pilot."
                    if agg["production_adopt_candidate"]
                    else (
                        "Continue shadow monitoring; pass checks incomplete."
                        if agg["production_shadow_continue"]
                        else "Do not adopt; shadow validation failed pass checks."
                    )
                ),
            },
        }
