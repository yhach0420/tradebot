"""FULL14 boards → P4-0 checkpoint_state plus 600s casebook horizon. No EXIT."""
...
from research.mid_hold_recovery_failure_path_p4_2 import CACHE_VERSION, CHECKPOINTS_SEC, EXIT600_REASON, EXTEND_REASON
from research.mid_hold_state_separability_p4_0 import IMBALANCE_REASON
...
DAY_CACHE = NATIVE / "results" / "research" / "_p4_2_day_cache"
...
def _labels(tr: dict[str, Any], *, top10: set[str], top20: set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "holding_sec": hold,
        "pnl_yen_100": p,
        "early_imbalance": reason == IMBALANCE_REASON and hold + 1e-12 < 600.0,
    }
    ...
    try:
        ...
        for tr in trades:
            ...
            for h in CHECKPOINTS_SEC:
                ...
                if rec["eligible"] and rec.get("bid_t") and rec.get("min_bid_since_fill"):
                    mn = float(rec["min_bid_since_fill"])
                    if mn > 0:
                        rec["rebound_from_low_t"] = float(rec["bid_t"]) / mn - 1.0
