"""Publish E1_X19 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "PopulationContract", "OutcomeClasses", "AnchorContract",
    "FeatureContract", "ContextStrata", "DateSplit", "MatchedGroups", "ClassProfiles",
    "SingleFeatureResults", "DailyBalance", "SymbolBalance", "LODO", "LOSO",
    "StressDay20260803", "MechanismGrouping", "DuplicateAudit", "StabilityGate",
    "KeyAnswers", "NextDecision", "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "stable_n", "value": len(report.get("stable_discriminators") or [])},
        {"item": "next", "value": (report.get("next_decision") or {}).get("action")},
    ]
    ...
    md = [
        ...
        f"- population: `{report.get('population_n')}` · classes: `{report.get('class_counts')}`",
        f"- stable discriminators: `{report.get('stable_discriminators')}`",
        f"- next: `{(report.get('next_decision') or {}).get('action')}`",
        f"- 20260804: UNCLASSIFIED_DO_NOT_OPEN",
        f"- tests: {tests.get('passed')}/{tests.get('total')} · A/B: {det.get('ab_match')}",
        "- submit/cancel/live: 0/0/0",
        "",
        "No ENTRY candidate / threshold / model created.",
        "",
    ]
