"""CLI: E1_X10 Risk Universe A/B + publish; stop."""
...
from research.e1_x10_risk_universe.publish import publish
from research.e1_x10_risk_universe.run_audit import PUBLISH, run_once
...
def _run_tests() -> dict:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "-rA", "--tb=line", "-p", "no:cacheprovider",
         str(native / "tests" / "test_e1_x10_risk_universe.py")],
...
def main() -> int:
    ...
    if str(report_a.get("verdict")).endswith("IDENTITY_MISMATCH"):
        print("IDENTITY MISMATCH", report_a.get("mismatches"), flush=True)
        return 2
    ...
    keys = [
        "source_identity_sha", "reference_price_sha", "notional_sha", "tick_risk_sha",
        "spread_sha", "depth_sha", "freshness_sha", "gap_risk_sha", "executable_loss_sha",
        "symbol_risk_summary_sha", "eligibility_sha", "verdict",
    ]
