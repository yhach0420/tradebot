"""Publish E1_X23 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "X21TouchNormalization", "ControlMismatch3",
    "CandidateRegistry", "DecisionMaskRegistry", "PairEvaluation", "ComponentFamilies",
    "PeriodTags", "RetentionBands", "BundleConstruction", "BundleAudit", "Precommit",
    "PrecommitIntegrity", "ProspectivePopulation", "ProspectivePairResults",
    "ProspectiveFamilyResults", "ProspectiveExitResults", "ProspectiveRetentionResults",
    "BundleVerdict", "CanonicalExitStatus", "ReservedDates", "Tests", "Determinism",
    "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "bundle_n", "value": report.get("bundle_pair_count")},
        {"item": "precommit_sha", "value": (report.get("precommit") or {}).get("bundle_sha256")},
        {"item": "prospective", "value": json.dumps(report.get("prospective_status_counts"))},
    ]
    ...
    md = [
        ...
        f"- bundle pairs / unique masks: `{report.get('bundle_pair_count')}` / `{report.get('bundle_unique_masks')}`",
        f"- precommit SHA: `{(report.get('precommit') or {}).get('bundle_sha256')}`",
        f"- raw-open-before-precommit: `{(report.get('precommit') or {}).get('20260804_raw_opened_before_precommit')}`",
        f"- 20260804 population: `{report.get('prospective_population_n')}`",
        f"- prospective status: `{report.get('prospective_status_counts')}`",
        f"- canonical: `CANONICAL_EXIT_PARITY_NOT_ESTABLISHED`",
