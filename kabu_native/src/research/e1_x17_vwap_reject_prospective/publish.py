"""Publish E1_X17 three artifacts."""
...
SHEETS = [
    "Index", "SealIntegrity", "PrecommitIdentity", "SourceIdentity", "C0Construction",
    "CandidateContract", "PrimaryOutcomes", "RejectedOutcomes", "HistoricalComparison",
    "FreshnessDiagnostics", "A3A4Diagnostics", "ProspectiveGate", "Tests", "Determinism",
    "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "seal_mode", "value": report.get("seal_mode")},
        {"item": "target_day", "value": report.get("target_day")},
    ]
    ...
    md = [
        ...
        f"- seal_mode: `{report.get('seal_mode')}` (not realtime Prospective)",
        f"- target_day: `{report.get('target_day')}`",
        f"- C0/A2/Rejected support: `{report.get('supports')}`",
        f"- precommit SHA match: `{report.get('precommit_sha_match')}`",
        f"- tests: {tests.get('passed')}/{tests.get('total')} · A/B: {det.get('ab_match')}",
        "- submit/cancel/live: 0/0/0",
        "",
        "Do not describe as production ready / robust / live ready.",
        "",
    ]
