"""
Phase 26: Early adverse move analysis and v4 comparison outputs.
"""
...
from research.entry_v2 import ENTRY_V2_PHASE26_PROFILES, MOMENTUM_V2_REFERENCE
from research.momentum_early_move import build_early_move_analysis
...
def _profile_metrics(profile_row: dict[str, Any], trade_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    early = sum(1 for r in rows if str(r.get("exit_reason")) == "early_adverse_guard")
    recovered = sum(1 for r in rows if r.get("recovered_after_adverse"))
    flushed = sum(1 for r in rows if r.get("had_adverse_flush"))
    return {
        **profile_row,
        ...
        "early_adverse_exit_rate": (early / n) if n else None,
        "early_adverse_recovery_rate": (recovered / flushed) if flushed else None,
    }
...
def _adoption_vs_v2(row: dict[str, Any], ref: dict[str, Any]) -> dict[str, Any]:
    ...
    if imb is not None and rimb is not None and imb > rimb + 0.03:
...
def write_momentum_phase26_outputs(
    out: Path,
    *,
    ...
    for pname in ENTRY_V2_PHASE26_PROFILES:
    ...
    if v2_rows:
        analysis = build_early_move_analysis(v2_rows)
        (out / "momentum_v4_early_move_analysis.json").write_text(
    ...
    with (out / "momentum_v4_comparison.csv").open("w", encoding="utf-8", newline="") as f:
        w = csv.DictWriter(f, fieldnames=fields, extrasaction="ignore")
        w.writeheader()
        w.writerows(flat)

    (out / "momentum_v4_comparison.json").write_text(
        json.dumps(
            {
                "phase": 26,
                "reference_profile": MOMENTUM_V2_REFERENCE,
                "profiles": list(ENTRY_V2_PHASE26_PROFILES),
                "rows": comp_rows,
            },
            ensure_ascii=False,
            indent=2,
