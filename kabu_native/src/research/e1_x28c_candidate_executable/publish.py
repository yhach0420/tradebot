"""Publish E1_X28C three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "ManifestIntegrity", "X28ExecutionParity", "BoardMapping",
    "QuoteContract", "EntryRegistry", "CandidateExitRegistry", "FamilyBaselineRegistry",
    "PeriodRoles", "ExecutionBridge", "FullExecutableReplay", "BidMarkSensitivity",
    "ExecutionCoverage", "CostDecomposition", "CandidateExecutableMetrics",
    "EntrySelectionExecutable", "PersonalizationExecutable", "ReferenceToExecutable",
    "Support", "Classification", "TargetVsTrail", "HorizonAnalysis", "StopRiskAnalysis",
    "ReferenceCIDiagnostic", "ExecutableBootstrap", "FDR", "DailySupport", "LODO", "LOSO",
    "DependencyClassification", "Stress20260803", "Consumed20260804", "PriorityTiers",
    "Views", "X29Handoff", "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "exec_joint", "value": report.get("EXECUTABLE_SPECIFIC_DIRECTIONAL_JOINT_POSITIVE")},
        {"item": "priority_a", "value": report.get("PRIORITY_A")},
    ]
    ...
    md = [
        ...
        f"- X28 parity: `{report.get('x28_parity_ok')}`",
        f"- exec JOINT_POSITIVE: `{report.get('EXECUTABLE_SPECIFIC_DIRECTIONAL_JOINT_POSITIVE')}`",
        f"- PRIORITY_A / B / FDR_CORE: `{report.get('PRIORITY_A')}` / "
        f"`{report.get('PRIORITY_B')}` / `{report.get('FDR_CORE')}`",
        f"- reference 266 survivors: `{report.get('ref_joint_survived_full')}`",
        f"- X29 handoff: `{report.get('x29_handoff_assignments')}`",
        f"- LODO/LOSO complete: `{report.get('LODO_complete')}` / `{report.get('LOSO_complete')}`",
        f"- tests: {tests.get('passed')}/{tests.get('total')} · A/B: {det.get('ab_match')}",
        "- submit/cancel/live: 0/0/0 · brokerage_fee_yen=0",
        "",
    ]
