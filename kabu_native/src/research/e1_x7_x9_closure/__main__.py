"""CLI: E1_X7–X9 Research Program Closure A/B + publish; stop."""
...
from research.e1_x7_x9_closure.assemble import PUBLISH, assemble
from research.e1_x7_x9_closure.publish import publish
...
def _run_tests() -> dict:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "-rA", "--tb=line", "-p", "no:cacheprovider",
         str(native / "tests" / "test_e1_x7_x9_closure.py")],
...
def main() -> int:
    print("=== E1_X7–X9 Closure tests ===", flush=True)
    tests = _run_tests()
    ...
    report_a = assemble(label="A")
    if report_a.get("verdict") == "E1_X7_X9_CLOSURE_SOURCE_IDENTITY_MISMATCH":
        print("SOURCE IDENTITY MISMATCH", report_a.get("mismatches"), flush=True)
        return 2

    print("=== Run B ===", flush=True)
    report_b = assemble(label="B")
