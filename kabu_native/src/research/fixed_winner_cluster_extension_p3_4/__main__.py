"""P3-4 runner. Winner cluster + 600s extension gate only. No new strategy."""
...
from research.fixed_winner_cluster_extension_p3_4 import FULL14, MAX_WORKERS, P1_PF, P1_PNL, P1_TRADES
from research.fixed_winner_cluster_extension_p3_4.publish import build_report, write_artifacts
from research.fixed_winner_cluster_extension_p3_4.replay import DAY_CACHE, replay_day_cached
...
def _blocked(primary, reason, failed, recon) -> int:
    rep = build_report(
        primary=primary,
        path_rows=[],
        recon=recon,
    ...
    print("P3_4_BLOCKED", reason, paths["report_json"], flush=True)
    return 1
...
def main() -> int:
    if not P1_REPORT.is_file():
        ...
        return _blocked([], "NO_P1_REPORT", ["NO_P1"], recon)
    ...
    print(
        f"P3-4 reconcile pass={recon['pass']} n={recon['trade_n']} pnl={recon['sum_pnl']} reasons={recon['reasons']}",
        flush=True,
    )
    if not recon["pass"]:
        return _blocked(primary, "P3_4_BLOCKED", recon["reasons"], recon)
    ...
    for day in FULL14:
        ...
        if not row or not row.get("capture_path") or not row.get("universe_symbols"):
            return _blocked(primary, f"MISSING_{day}", [day], recon)
        jobs.append(
            {
                "date": day,
    ...
    print(f"P3-4 days={len(jobs)} workers={MAX_WORKERS} canonical={len(primary)}", flush=True)
    results = []
    failed = []
    ...
    if failed or len(results) != 14:
        return _blocked(primary, "DAY_REPLAY_FAILED", failed, recon)
    ...
    paths = write_artifacts(rep)
    if str(rep.get("verdict")) == "P3_4_WINNER_CLUSTER_EXTENSION_AUDIT_COMPLETE":
        for p in DAY_CACHE.glob("*.json"):
            p.unlink(missing_ok=True)
    print(rep["verdict"], paths["report_json"], flush=True)
    print(
        f"RECON={rep['CANONICAL_RECONCILE']} DEC={rep['EXTENSION_DECISION_RECONCILE']} "
        f"CLUSTER={rep['WINNER_CONCENTRATION']} GATE={rep['EXTENSION_GATE']} "
        f"MECH={rep['EXTENSION_MECHANISM']} LEAK={rep['FUTURE_LEAK']}",
        flush=True,
    )
