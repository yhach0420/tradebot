"""Discover and split canonical-reconstructable days (v2)."""
...
from research.canonical_zero_base_v2.constants import CAPTURE_ROOT, LOT
...
def list_capture_days(root: Path = CAPTURE_ROOT) -> list[str]:
    ...
    return sorted(
        ...
        if p.is_dir() and any(p.glob("push_part_*.jsonl"))
    )


def audit_day(day: str, root: Path = CAPTURE_ROOT, sample_limit: int = 8000) -> dict[str, Any]:
    day_dir = root / day
    n = n_buy1 = n_sell1 = n_ask = n_bid = n_px = n_vol = n_map = 0
    n_crossed = n_locked = n_spread = n_exec100 = am = pm = 0
    for fp in sorted(day_dir.glob("push_part_*.jsonl")):
        with fp.open("r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                if (aq is not None and aq >= LOT) or (bq is not None and bq >= LOT):
                    n_exec100 += 1
                if (
    ...
    formal = bool(n > 0 and ask_cov >= 0.90 and bid_cov >= 0.90 and map_rate >= 0.90 and (n_px / n) >= 0.90)
    if formal:
        tier = "FORMAL"
    elif ask_cov >= 0.70 and bid_cov >= 0.70 and map_rate >= 0.70:
        tier = "PARTIAL"
    else:
        tier = "UNUSABLE"
    return {
        ...
        "spread_valid_rate": n_spread / n if n else 0.0,
        "exec100_coverage": n_exec100 / n if n else 0.0,
        "am_samples": am,
        "pm_samples": pm,
...
def discover_and_split(root: Path = CAPTURE_ROOT) -> dict[str, Any]:
    ...
    # fix list_capture_days logic for glob
    if not days:
        days = sorted(p.name for p in root.iterdir() if p.is_dir() and list(p.glob("push_part_*.jsonl")))
    audits = [audit_day(d, root=root) for d in days]
    eligible = [a["day"] for a in audits if a["formal_eligible"] or a["tier"] in ("FORMAL", "PARTIAL")]
    if not eligible:
        eligible = list(days)
    ...
    if insufficient:
        if len(eligible) >= 4:
            warmup, oos = [eligible[0]], [eligible[-1]]
            mid = eligible[1:-1]
            ...
            warmup = train = val = [eligible[0]]
            oos = [eligible[1]]
        else:
            warmup = train = val = oos = list(eligible)
    ...
    return {
        ...
        "bid_coverage_mean": sum(a["bid_coverage"] for a in audits) / len(audits) if audits else 0.0,
    }
