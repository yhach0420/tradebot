"""Load Watch50 PUSH streams with canonical Buy1/Sell1 boards."""
...
from research.canonical_zero_base_v2.constants import CAPTURE_ROOT, SAMPLE_STRIDE
...
class Tick:
    ...
    exchange_time: str = ""
    received_at: str = ""
    depth_bid: list[tuple[Optional[float], Optional[float]]] = field(default_factory=list)
    depth_ask: list[tuple[Optional[float], Optional[float]]] = field(default_factory=list)
    idx: int = 0


def _depth(op: dict, side: str) -> list[tuple[Optional[float], Optional[float]]]:
    out = []
    for i in range(1, 11):
        lv = op.get(f"{side}{i}")
        if isinstance(lv, dict):
            out.append((_f(lv.get("Price")), _f(lv.get("Qty"))))
        else:
        ...
        with fp.open("r", encoding="utf-8", errors="ignore") as f:
            for i, line in enumerate(f):
                ...
                yield Tick(
                    ...
                    exchange_time=str(op.get("CurrentPriceTime") or ""),
                    received_at=str(rec.get("received_at_jst") or ""),
                    depth_bid=_depth(op, "Buy"),
                    depth_ask=_depth(op, "Sell"),
                )
