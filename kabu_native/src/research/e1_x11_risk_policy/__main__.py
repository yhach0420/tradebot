"""CLI: E1_X11 Risk Policy Calibration A/B + publish; stop."""
...
from research.e1_x11_risk_policy.publish import publish
from research.e1_x11_risk_policy.run_audit import PUBLISH, run_once
...
def _run_tests() -> dict:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "pytest", "-q", "-rA", "--tb=line", "-p", "no:cacheprovider",
         str(native / "tests" / "test_e1_x11_risk_policy.py")],
...
def main() -> int:
    ...
    if "MISMATCH" in str(report_a.get("verdict")):
        print("MISMATCH", report_a.get("mismatches"), flush=True)
        return 2
    ...
    keys = [
        "source_identity_sha", "policy_definition_sha", "capital_base_sha",
        "rolling_support_sha", "recurring_sha", "scenario_sha", "kioxia_sha", "verdict",
    ]
