"""P3-4R runner. Decision-time alignment only. P3-4 cluster frozen. No new strategy."""
...
from research.extension_decision_alignment_p3_4r import FULL14, MAX_WORKERS, P1_PF, P1_PNL, P1_TRADES
from research.extension_decision_alignment_p3_4r.publish import build_report, load_frozen_p3_4, write_artifacts
from research.extension_decision_alignment_p3_4r.replay import DAY_CACHE, replay_day_cached
...
def _blocked(primary, reason, failed, recon, frozen) -> int:
    rep = build_report(
        path_rows=[],
        recon=recon,
        frozen=frozen,
    ...
    print("P3_4R_BLOCKED", reason, paths["report_json"], flush=True)
    return 1


def main() -> int:
    frozen = load_frozen_p3_4()
    ...
    print(
        f"P3-4R reconcile pass={recon['pass']} n={recon['trade_n']} pnl={recon['sum_pnl']} "
        f"frozen={frozen.get('ok')} reasons={recon['reasons']}",
        flush=True,
    )
    if not recon["pass"]:
        return _blocked(primary, "P3_4R_BLOCKED", recon["reasons"], recon, frozen)
    if not frozen.get("ok"):
        return _blocked(primary, frozen.get("reason") or "P3_4_FROZEN_MISSING", [], recon, frozen)
    ...
    for day in FULL14:
        ...
        if not row or not row.get("capture_path") or not row.get("universe_symbols"):
            return _blocked(primary, f"MISSING_{day}", [day], recon, frozen)
        jobs.append(
            {
                "date": day,
    ...
    print(f"P3-4R days={len(jobs)} workers={MAX_WORKERS} canonical={len(primary)}", flush=True)
    results = []
    failed = []
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            if not out.get("ok"):
                ...
                failed.append(day)
                continue
            n_r = sum(1 for r in (out.get("rows") or []) if r.get("reached_600"))
            n_m = sum(1 for r in (out.get("rows") or []) if r.get("matched") is True)
            src = "cache" if out.get("from_cache") else "stream"
            print(
                f"OK {day} [{src}] path={len(out.get('rows') or [])}/{out.get('n_canonical')} "
                f"reached600={n_r} match={n_m} "
                f"leak={out.get('leak_decision')}/{out.get('leak_checkpoint')} "
                f"overlap={out.get('overlap_n')} ge750={out.get('n_decision_ge_750')} "
    ...
    if failed or len(results) != 14:
        return _blocked(primary, "DAY_REPLAY_FAILED", failed, recon, frozen)
    ...
    rep = build_report(
        ...
        recon=recon,
        frozen=frozen,
        leak_decision=sum(int(d.get("leak_decision") or 0) for d in results),
        leak_checkpoint=sum(int(d.get("leak_checkpoint") or 0) for d in results),
        overlap_n=sum(int(d.get("overlap_n") or 0) for d in results),
        n_decision_ge_750=sum(int(d.get("n_decision_ge_750") or 0) for d in results),
    ...
    paths = write_artifacts(rep)
    if str(rep.get("verdict")) == "P3_4R_EXTENSION_DECISION_ALIGNMENT_RECONCILED":
        for p in DAY_CACHE.glob("*.json"):
            p.unlink(missing_ok=True)
    print(rep["verdict"], paths["report_json"], flush=True)
    print(
        f"RECON={rep['CANONICAL_RECONCILE']} DEC={rep['DECISION_RECONCILE']} "
        f"GATE={rep['EXTENSION_GATE_REVISED']} MECH={rep['EXTENSION_MECHANISM_REVISED']} "
        f"OVERLAP={rep['DECISION_OUTCOME_OVERLAP']} LEAK={rep['FUTURE_LEAK']}",
        flush=True,
    )
