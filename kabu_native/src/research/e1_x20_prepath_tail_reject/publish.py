"""Publish E1_X20 three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "FeatureContract", "ThresholdContract", "QuantileBins",
    "Monotonicity", "ThresholdTransport", "VariantDefinitions", "ClassComposition",
    "SingleMechanism", "TwoMechanismIncrement", "PeriodResults", "DailyResults",
    "MatchedGroups", "SymbolResults", "LODO", "LOSO", "CandidateSelection",
    "ProspectivePrecommit", "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "precommit", "value": report.get("prospective_precommit_status")},
    ]
    ...
    md = [
        ...
        f"- thresholds: slope_q80=`{report.get('thresholds', {}).get('SLOPE_UPPER_LIMIT')}` · rebound_q80=`{report.get('thresholds', {}).get('REBOUND_UPPER_LIMIT_BPS')}`",
        f"- selected: `{report.get('selected_candidate')}`",
        f"- precommit: `{report.get('prospective_precommit_status')}`",
        f"- 20260804 registry: `{report.get('registry_20260804_status')}`",
        f"- tests: {tests.get('passed')}/{tests.get('total')} · A/B: {det.get('ab_match')}",
        "- submit/cancel/live: 0/0/0 · not production ready",
        "",
    ]
