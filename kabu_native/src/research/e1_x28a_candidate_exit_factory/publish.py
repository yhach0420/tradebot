"""Publish E1_X28A three artifacts."""
...
SHEETS = [
    "Index", "SourceIdentity", "RegistryIntegrity", "DiscoveryDataContract",
    "CandidatePathMetrics", "CandidateCalibrationSupport", "HorizonDerivation",
    "ModeDerivation", "ParameterGrids", "TargetExitCalibration", "TrailExitCalibration",
    "StopCalibration", "NoProgressCalibration", "FallbackRegistry",
    "CandidateExitAssignments", "SemanticExitRegistry", "SemanticAliases",
    "StopRiskTags", "DiscoveryTriggerReplay", "ExitReasonCoverage",
    "X28BaselineLimitations", "X28BComparisonRegistry", "Manifest",
    "Tests", "Determinism", "Safety", "ChangeLog",
]
...
def publish(report: dict[str, Any], tests: dict[str, Any], det: dict[str, Any], out_dir: Path) -> dict[str, str]:
    ...
    sheets["Index"] = [
        ...
        {"item": "manifest_sha", "value": report.get("manifest_sha256")},
        {"item": "assignments", "value": report.get("assignments")},
        {"item": "candidate_specific", "value": report.get("candidate_specific_count")},
    ]
    ...
    md = [
        ...
        f"- manifest SHA: `{report.get('manifest_sha256')}`",
        f"- assignments: `{report.get('assignments')}`",
        f"- candidate-specific / family / control: "
        f"`{report.get('candidate_specific_count')}` / "
        f"`{report.get('family_fallback_count')}` / "
        f"`{report.get('control_fallback_count')}`",
        f"- unique semantic EXITs: `{report.get('unique_semantic_exit_count')}`",
        f"- TARGET / TRAIL: `{report.get('target_count')}` / `{report.get('trail_count')}`",
