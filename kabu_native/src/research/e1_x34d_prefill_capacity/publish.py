...
def write_md(path: Path, report: dict[str, Any]) -> None:
    lines = [
        ...
        f"- X34C qualification: `{report.get('x34c_qualification')}`",
        "",
        f"- U0/C1/C2: `{report.get('baselines_summary')}`",
        f"- capacity drag: `{report.get('capacity_drag')}`",
        f"- ordering sensitivity: `{report.get('ordering_sensitivity')}`",
        "",
