"""P4-3 runner. Precommit SHA first, then local continuation-value audit. No Gate / Runtime."""
...
from research.mid_hold_continuation_value_p4_3 import FULL14, MAX_WORKERS, P1_PF, P1_PNL, P1_TRADES, VERDICT_OK
from research.mid_hold_continuation_value_p4_3.precommit import write_precommit
from research.mid_hold_continuation_value_p4_3.publish import build_report, load_frozen, write_artifacts
from research.mid_hold_continuation_value_p4_3.replay import DAY_CACHE, replay_day_cached
...
def _blocked(reason, recon, frozen, pre) -> int:
    rep = build_report(
        precommit=pre, recon=recon, frozen=frozen, rows=[], leak_n=0, identity_n=0, identity_fail=0,
        blocked=True, blocked_reason=reason,
    )
    paths = write_artifacts(rep)
    print("P4_3_BLOCKED", reason, paths["report_json"], flush=True)
    return 1


def main() -> int:
    pre = write_precommit()
    print(f"P4-3 PRECOMMIT_SHA={pre.get('SHA')}", flush=True)
    frozen = load_frozen()
    if not P1_REPORT.is_file():
        return _blocked("NO_P1_REPORT", {"pass": False, "reasons": ["NO_P1"]}, frozen, pre)
    ...
    print(
        f"P4-3 reconcile pass={recon['pass']} n={recon['trade_n']} pnl={recon['sum_pnl']} frozen={frozen.get('ok')}",
        flush=True,
    )
    if not recon["pass"] or not frozen.get("ok"):
        return _blocked("CANONICAL_OR_FROZEN_FAIL", recon, frozen, pre)
    ...
    for day in FULL14:
        ...
        if not row or not row.get("capture_path") or not row.get("universe_symbols"):
            return _blocked(f"MISSING_{day}", recon, frozen, pre)
        jobs.append(
            {
                "date": day,
    ...
    print(f"P4-3 days={len(jobs)} workers={MAX_WORKERS} canonical={len(primary)}", flush=True)
    results = []
    failed = []
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as ex:
        ...
        for fut in as_completed(futs):
            ...
            print(
                f"OK {day} [{src}] rows={len(out.get('rows') or [])} leak={out.get('leak_n')} "
    ...
    if failed or len(results) != 14:
        return _blocked("DAY_REPLAY_FAILED", recon, frozen, pre)
    ...
    print(
        f"SHA={rep.get('PRECOMMIT_SHA')} CLASS={rep.get('ACTIONABILITY_CLASSIFICATION')} "
        f"COHORT={rep.get('PRIMARY_COHORT')} LEAK={rep.get('FUTURE_LEAK')}",
        flush=True,
    )
    return 0 if str(rep["verdict"]) != "P4_3_BLOCKED" else 1
