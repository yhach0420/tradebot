"""
Phase361: Near day high + low momentum ENTRY guard shadow (C03 from Phase360).

Block shadow counterfactual when:
  - day_high_distance_pct <= 1.5
  - AND entry_momentum_score < 0.30
"""
...
GUARD_VARIANT = "C03_near_day_high_low_mom_guard"

DAY_HIGH_DISTANCE_MAX_PCT = 1.5
MOMENTUM_MAX_EXCLUSIVE = 0.30
...
def would_block_near_day_high_low_mom_guard(fields: Mapping[str, Any]) -> bool:
    dist = _day_high_distance(fields)
    mom = _entry_momentum(fields)
    if dist is None or mom is None:
        return False
    return dist <= DAY_HIGH_DISTANCE_MAX_PCT and mom < MOMENTUM_MAX_EXCLUSIVE
...
def enrich_trade_for_near_day_high_low_mom_shadow(
    trade: Mapping[str, Any],
    acc: Mapping[str, str],
) -> dict[str, Any]:
    dist = _day_high_distance({**acc, **trade})
    mom = _entry_momentum({**acc, **trade})
    fields = {
        "day_high_distance_pct": dist,
        "entry_near_day_high_pct": dist,
        "entry_momentum_score": mom,
        "entry_momentum_continuation_score": _float(
            acc.get("entry_momentum_continuation_score") or trade.get("entry_momentum_continuation_score")
        ),
    ...
    blocked = would_block_near_day_high_low_mom_guard(fields)
    ...
    return {
        **dict(trade),
        "day_high_distance_pct": dist,
        "entry_momentum_score": mom,
        "near_day_high_low_mom_guard_shadow_blocked": blocked,
        "near_day_high_low_mom_shadow_pnl_yen_100": shadow_yen,
        "near_day_high_low_mom_shadow_delta_yen": round(shadow_yen - (yen or 0.0), 2)
        if yen is not None
        else None,
...
def evaluate_session(session_meta: Mapping[str, Any], *, reports_dir: Any) -> dict[str, Any]:
    ...
    for trade in base.get("kept") or []:
        ...
        t = enrich_trade_for_near_day_high_low_mom_shadow(trade, acc)
