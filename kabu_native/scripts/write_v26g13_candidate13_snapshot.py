#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G13 Candidate-13 snapshot from current working tree.

Does not mutate Formal V25 or Candidate-6/7/8/9/10/11/12. Does not Formal-freeze V26.
Does not enable Formal Paper. OPVAL current-trading-day identity is not rewritten.
Candidate-10 DualLane bytes and Candidate-12 snapshot must remain identical.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G13_13"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g13_13.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-13 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(c12.get(key) or "") != str(v25.get(key) or "") or str(c12.get(key) or "") != str(c10.get(key) or ""):
            print("REFUSE: strategy identity drift vs V25/C12/C10", key)
            return 2

    cov = audit_runtime_inventory_coverage(native_root=NATIVE)
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G13_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    drifted = [
        ...
        and rel in c12_inv
        and str(c12_inv.get(rel) or "") != str(inv.get(rel) or "")
    ]
    if drifted:
        print("REFUSE: unexpected inventory drift vs Candidate-12 G=", len(drifted), drifted[:20])
        return 2
    new_keys = [rel for rel in RUNTIME_DEPENDENCY_RELS if rel not in c12_inv]
    if any(rel not in ALLOWED_RUNTIME_DRIFT for rel in new_keys):
        print("REFUSE: unexpected new inventory keys vs Candidate-12", new_keys)
        return 2
    ...
    if "src/small_paper/evaluation_reachability.py" not in runtime_changed:
        print("REFUSE: expected evaluation_reachability O(1) counter vs Candidate-12")
        return 2
    if "src/small_paper/pilot_runner.py" not in runtime_changed:
        print("REFUSE: expected pilot_runner O(1) wiring vs Candidate-12")
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G13_REACHABILITY_O1_EXACT_COUNTER",
            ...
            "parent_runtime_candidate_id": C12_ID,
            "parent_runtime_candidate_sha": C12_SHA,
            "parent_runtime_candidate_status": "UNCERTIFIED_IMMUTABLE_LEFT_IN_PLACE",
            "allowed_runtime_drift_vs_candidate12": sorted(ALLOWED_RUNTIME_DRIFT),
            ...
            "candidate12_unchanged": True,
        }
    )
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(body.get(key) or "") != str(c12.get(key) or ""):
            print("REFUSE: strategy SHA != Candidate-12", key, body.get(key), c12.get(key))
            return 2
    ...
    selector = {
        ...
        "note": (
            "Identity-only UNCERTIFIED candidate-13 selector; not the active Formal selector. "
            "Not Candidate-12. OPVAL current-trading-day identity is not rewritten."
        ),
    }
    ...
    c12_after = json.loads((OUT / f"{C12_ID}.json").read_text(encoding="utf-8"))
    if v25_after.get("sha256") != V25_SHA or sel_after.get("activation_id") != V25_ACTIVATION_ID:
        ...
        return 2
    if c12_after.get("sha256") != C12_SHA:
        print("REFUSE: Candidate-12 mutated during candidate write")
        dest.unlink(missing_ok=True)
        SELECTOR_CANDIDATE.unlink(missing_ok=True)
        return 2
    ...
    print("CANDIDATE12_UNCHANGED=true")
