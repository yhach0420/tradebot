#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V10.

Parent V9 is immutable history. Does not mutate Strategy / Precommit / V9–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V9"
PARENT_SHA = "858f21fa785ac508b92b8d54c10a7c6ec7cb3483157b80936caf167e9684f7f1"
V10_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V10"
SUPERSEDE_REASON = "RUNTIME_NATIVE_FULL_PUSH_INGEST_AND_PASSIVE_FILL_PARITY_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V10_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v9 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v9["sha256"] == PARENT_SHA == manifest_content_sha(v9), "V9 must remain immutable"
    ...
    activation = {k: v for k, v in v9.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V10_ID,
    ...
    assert activation["guard"] == v9["guard"]
    assert activation["continuation"] == v9["continuation"]
    assert activation["cap"] == v9["cap"]
    assert activation["wait_sec"] == v9["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V10_ID}.json"
    ...
    v9_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v9_after["sha256"] == PARENT_SHA == manifest_content_sha(v9_after)
    ...
    selector = {
        ...
        "activation_id": V10_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V10_ID}.json",
    ...
    assert sel["activation_id"] == V10_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V10_FROZEN",
        "activation_id": V10_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v10.json").write_text(
