#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G10 Candidate-10 snapshot from current working tree.

Does not mutate Formal V25 or Candidate-6/7/8/9. Does not Formal-freeze V26.
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G10_10"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g10_10.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-10 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    strategy_ok = (
        str(v25.get("strategy_sha") or "") == str(c9.get("strategy_sha") or "") == str(c6.get("strategy_sha") or "")
    ...
    if not strategy_ok:
        print("REFUSE: strategy identity drift vs Candidate-6/9 pins")
        return 2

    cov = audit_runtime_inventory_coverage(native_root=NATIVE)
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G10_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    if len(inv) != len(RUNTIME_DEPENDENCY_RELS):
        ...
        return 2
    c9_inv = c9.get("runtime_file_sha256") or {}
    drifted = [
        rel
        for rel in RUNTIME_DEPENDENCY_RELS
        if rel not in ALLOWED_RUNTIME_DRIFT and str(c9_inv.get(rel) or "") != str(inv.get(rel) or "")
    ]
    if drifted:
        print("REFUSE: strategy-affecting inventory drift G=", len(drifted), drifted[:20])
        return 2
    runtime_changed = [rel for rel in sorted(ALLOWED_RUNTIME_DRIFT) if str(c9_inv.get(rel) or "") != str(inv.get(rel) or "")]
    if "src/small_paper/v1r_live_dual_lane.py" not in runtime_changed:
        print("REFUSE: expected DualLane runtime drift vs Candidate-9")
        return 2
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(c9.get(key) or "") != str(c7.get(key) or "") or str(c9.get(key) or "") != str(v25.get(key) or ""):
            print("REFUSE: Candidate-9 strategy identity drifted vs V25/C7", key)
            return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G10_DUALLANE_TRUE_INCREMENTAL_LARGE_BOARD_REPAIR",
            ...
            "parent_runtime_candidate_id": C9_ID,
            "parent_runtime_candidate_sha": C9_SHA,
            "parent_runtime_candidate_status": "UNCERTIFIED_IMMUTABLE_LEFT_IN_PLACE",
            "allowed_runtime_drift_vs_candidate9": sorted(ALLOWED_RUNTIME_DRIFT),
            "runtime_changed_rels": runtime_changed,
        }
    )
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(body.get(key) or "") != str(c9.get(key) or ""):
            print("REFUSE: strategy SHA != Candidate-9", key, body.get(key), c9.get(key))
            return 2
    ...
    selector = {
        ...
        "note": (
            "Identity-only UNCERTIFIED candidate-10 selector; not the active Formal selector. "
            "Not Candidate-9. OPVAL current-trading-day identity is not rewritten."
        ),
    }
    ...
    c9_after = json.loads((OUT / f"{C9_ID}.json").read_text(encoding="utf-8"))
    if v25_after.get("sha256") != V25_SHA or sel_after.get("activation_id") != V25_ACTIVATION_ID:
        ...
        return 2
    if c9_after.get("sha256") != C9_SHA:
        print("REFUSE: Candidate-9 mutated during candidate write")
        dest.unlink(missing_ok=True)
        SELECTOR_CANDIDATE.unlink(missing_ok=True)
        return 2
    ...
    print("CANDIDATE9_UNCHANGED=true")
