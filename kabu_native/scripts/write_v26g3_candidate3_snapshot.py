#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G3 Candidate-3 snapshot (48x lag/clock fix).

Does not mutate V25 selector/manifest.
Does not rewrite Candidate-1 (FAILED / AUDIT_ONLY).
Does not rewrite Candidate V26G3_2 (premature snapshot; immutable).
Does not Formal-freeze V26.
Refuses overwrite if Candidate-3 files already exist.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G3_3"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g3_3.json"
...
def main() -> int:
    ...
    cand1 = OUT / f"{CANDIDATE1_ID}.json"
    cand2 = OUT / f"{CANDIDATE2_ID}.json"
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-3 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    if not cand1.is_file():
        print("REFUSE: candidate-1 manifest missing")
        return 2
    c1 = json.loads(cand1.read_text(encoding="utf-8"))
    if c1.get("sha256") != CANDIDATE1_SHA or c1.get("candidate_id") != CANDIDATE1_ID:
        print("REFUSE: candidate-1 identity mutated")
        return 2
    if not cand2.is_file():
        print("REFUSE: candidate V26G3_2 manifest missing")
        return 2
    c2 = json.loads(cand2.read_text(encoding="utf-8"))
    if c2.get("sha256") != CANDIDATE2_SHA or c2.get("candidate_id") != CANDIDATE2_ID:
        print("REFUSE: candidate V26G3_2 identity mutated")
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G3_UNCERTIFIED_RUNTIME_FIX_CANDIDATE3_48X_LAG_CLOCK",
            ...
            "parent_premature_candidate_status": "UNCERTIFIED_PREFLIGHT_ABORTED_48X_CLOCK_RACE",
        }
    )
    ...
    selector = {
        ...
        "note": "Identity-only UNCERTIFIED candidate-3 selector; not the active Formal selector.",
    }
    ...
    c1_after = json.loads(cand1.read_text(encoding="utf-8"))
    c2_after = json.loads(cand2.read_text(encoding="utf-8"))
    if v25_after.get("sha256") != V25_SHA or sel_after.get("activation_id") != V25_ACTIVATION_ID:
        ...
        return 2
    if c1_after.get("sha256") != CANDIDATE1_SHA:
        print("REFUSE: candidate-1 mutated during candidate-3 write")
        return 2
    if c2_after.get("sha256") != CANDIDATE2_SHA:
        print("REFUSE: candidate V26G3_2 mutated during candidate-3 write")
        return 2
    ...
    print("CANDIDATE1_UNCHANGED=true")
    print("CANDIDATE_V26G3_2_UNCHANGED=true")
    return 0
