#!/usr/bin/env python3
"""Phase368: Symbol reentry cluster guard shadow validation."""
...
def _worker(job: dict) -> dict:
    ...
    from small_paper.symbol_reentry_cluster_entry_guard_shadow import (
        load_session_production_trades_for_shadow,
    )

    t0 = time.monotonic()
    try:
        result = load_session_production_trades_for_shadow(
            job["session_meta"],
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase368 symbol reentry cluster shadow")
    ...
    _bootstrap()
    from research.phase368_symbol_reentry_cluster_guard_validation import (
        MIN_DAY,
        Phase368SymbolReentryClusterValidation,
    )
    ...
    audit = Phase368SymbolReentryClusterValidation(reports_dir=REPORTS)
    jobs = [{"session_meta": meta, "reports_dir": str(REPORTS)} for meta in sessions]

    print(
        f"phase368 sessions={len(sessions)} parallel={args.parallel} "
