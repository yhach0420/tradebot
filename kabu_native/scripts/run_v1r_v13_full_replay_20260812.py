#!/usr/bin/env python
"""2026-08-12 Full Capture Replay after V13 frozen-universe SoT fix.

V12–V1 manifests are immutable history. Strategy/Precommit unchanged.
"""
...
OUT = ROOT / "results" / "research" / "v1r_v13_full_replay_20260812"
...
def v12_immutable_precheck() -> dict:
    v12 = json.loads((ACT_DIR / f"{V12_ID}.json").read_text(encoding="utf-8"))
    ok, _, calc = verify_manifest_self_sha(v12)
    checks = {
        "v12_self_sha": ok and v12.get("sha256") == V12_SHA == calc == manifest_content_sha(v12),
        "strategy": v12.get("strategy_sha") == STRATEGY_SHA,
        "precommit": v12.get("precommit_sha") == PRECOMMIT_SHA,
        "submit_cancel_live": True,
    }
    return {"ok": all(checks.values()), "checks": checks, "v12_content_sha": v12.get("sha256")}


def main() -> int:
    v4h = _load_v4_harness()
    v4h.OUT = OUT
    v4h.v3_immutable_precheck = v12_immutable_precheck  # type: ignore[method-assign]
    ...
    if report_path.is_file():
        ...
        if rep.get("verdict") == "V1R_V4_20260812_FULL_REPLAY_E2E_PASS":
            rep["verdict"] = "V1R_V13_20260812_FULL_REPLAY_E2E_PASS"
        elif rep.get("verdict") == "V1R_V4_20260812_FULL_REPLAY_E2E_FAIL":
            rep["verdict"] = "V1R_V13_20260812_FULL_REPLAY_E2E_FAIL"
        ...
        print("V13_REPLAY_VERDICT", rep.get("verdict"), "blockers", rep.get("blockers"), flush=True)
