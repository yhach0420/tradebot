#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V23.

Parent V22 is immutable history. Does not mutate Strategy / Precommit / V22–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V22"
PARENT_SHA = "6c2d05dde6f64b4838a6b69c009c34981bf054ad4a253d85aa3aefdd05f29d34"
V23_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V23"
SUPERSEDE_REASON = "RUNTIME_SESSION_DAY_IDENTITY_POSTSESSION_SCOPE_AND_CERT_STREAM_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V23_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V22 must remain immutable"
    ...
    dump_runtime_artifact_inventory(OUT / "V23_RUNTIME_ARTIFACT_DECISION_INVENTORY.json")
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V23 binds "
        "session-day identity, current-run post-session seal scope, stage token "
        "rollover, and Full-Day certification input coverage. V22 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V23_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V23_ID}.json",
    ...
    assert sel["activation_id"] == V23_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V23_FROZEN",
        "activation_id": V23_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v22_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v23.json").write_text(
