#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G6 Candidate-6 snapshot.

Run ONLY after A+B+C 48x, 1x sentinel, speed parity, and full regression PASS.
Does not mutate V25 or Candidate-5. Does not Formal-freeze V26. Does not enable Formal Paper.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G6_6"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g6_6.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-6 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G6_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G6_UNCERTIFIED_RUNTIME_FIX_CANDIDATE6",
            ...
            "parent_failed_candidate_id": "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G4_5",
            "parent_failed_candidate_sha": C5_SHA,
            "parent_failed_candidate_status": "UNCERTIFIED_FULL_RUNTIME_PREFLIGHT_FAIL",
        }
    )
    ...
    selector = {
        ...
        "note": "Identity-only UNCERTIFIED candidate-6 selector; not the active Formal selector. Not Candidate-5.",
    }
