#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V18.

Parent V17 is immutable history. Does not mutate Strategy / Precommit / V17–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V17"
PARENT_SHA = "fdbcca94c160204213083b78a8fc1bcf0fead50221149fc7c04e9a68af3fb7dc"
V18_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V18"
SUPERSEDE_REASON = "RUNTIME_FULL_DAY_CERT_TOKEN_AUTHORITY_SESSION_DAY"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V18_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V18 binds "
        "token-authority day to RuntimeClock so children cannot issue a second "
        "token against wall-clock today. V17 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V18_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V18_ID}.json",
    ...
    assert sel["activation_id"] == V18_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V18_FROZEN",
        "activation_id": V18_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v17_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v18.json").write_text(
