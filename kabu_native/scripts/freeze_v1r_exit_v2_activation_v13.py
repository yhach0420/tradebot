#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V13.

Parent V12 is immutable history. Does not mutate Strategy / Precommit / V12–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V12"
PARENT_SHA = "934c082e402020b9ac2d4b3c7d240a06bc67ebf9cd11d8d1d9b04214f5f11982"
V13_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V13"
SUPERSEDE_REASON = "RUNTIME_FROZEN_UNIVERSE_SOT_AND_PM_REBUILD_ISOLATION_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V13_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v12 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v12["sha256"] == PARENT_SHA == manifest_content_sha(v12), "V12 must remain immutable"

    v11 = json.loads((OUT / "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V11.json").read_text(encoding="utf-8"))
    assert v11["sha256"] == V11_SHA == manifest_content_sha(v11)
    ...
    activation = {k: v for k, v in v12.items() if k != "sha256"}
    invalid = dict(activation.get("invalid_prospective_days") or {})
    invalid["20260814"] = {
        "status": "V1R_20260814_PROSPECTIVE_DAY1_INVALID_RUNTIME_FAIL",
        "reason": (
            "Prior V12 consumer-lag death spiral plus V12 PM EMPTY_UNIVERSE from "
            "PM_REBUILD_OVERWROTE_FROZEN_SOURCE. PnL excluded from Strategy evaluation. "
            "V13 is the runtime frozen-SoT fix; not Prospective Day1."
        ),
    }
    activation.update(
        {
    ...
    assert activation["guard"] == v12["guard"]
    assert activation["continuation"] == v12["continuation"]
    assert activation["cap"] == v12["cap"]
    assert activation["wait_sec"] == v12["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V13_ID}.json"
    ...
    v12_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v12_after["sha256"] == PARENT_SHA == manifest_content_sha(v12_after)
    ...
    selector = {
        ...
        "activation_id": V13_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V13_ID}.json",
    ...
    assert sel["activation_id"] == V13_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V13_FROZEN",
        "activation_id": V13_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v13.json").write_text(
