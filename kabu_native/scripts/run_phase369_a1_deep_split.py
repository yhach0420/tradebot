#!/usr/bin/env python3
"""Phase369: A1 board-low low-momentum deep split."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase369 A1 deep split")
    ...
    from research.phase369_a1_deep_split import Phase369A1DeepSplit
    ...
    audit = Phase369A1DeepSplit(reports_dir=REPORTS)
    jobs = [{"session_meta": meta, "reports_dir": str(REPORTS)} for meta in sessions]

    print(
        f"phase369 sessions={len(sessions)} parallel={args.parallel} "
    ...
    print("\n=== Phase369 Summary ===", flush=True)
    print(f"a1_count: {summary.get('a1_trade_count')}", flush=True)
    print(f"a1_loss: {summary.get('a1_total_loss_yen_100')}", flush=True)
