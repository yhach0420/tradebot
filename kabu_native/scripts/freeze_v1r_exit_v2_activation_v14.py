#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V14.

Parent V13 is immutable history. Does not mutate Strategy / Precommit / V13–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V13"
PARENT_SHA = "36fcee66a59fa708699c33e1671470153bd78c275d2cd5428876dfd179f31c13"
V14_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V14"
SUPERSEDE_REASON = "RUNTIME_TEARDOWN_EXTERNAL_BACKUP_LOGGER_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V14_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v13 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v13["sha256"] == PARENT_SHA == manifest_content_sha(v13), "V13 must remain immutable"
    ...
    activation = {k: v for k, v in v13.items() if k != "sha256"}
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V14 is a "
        "runtime teardown logger fix only."
    )
    ...
    assert activation["guard"] == v13["guard"]
    assert activation["continuation"] == v13["continuation"]
    assert activation["cap"] == v13["cap"]
    assert activation["wait_sec"] == v13["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V14_ID}.json"
    ...
    v13_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v13_after["sha256"] == PARENT_SHA == manifest_content_sha(v13_after)
    v12_after = json.loads((OUT / f"{V12_ID}.json").read_text(encoding="utf-8"))
    assert v12_after["sha256"] == V12_SHA == manifest_content_sha(v12_after)
    ...
    selector = {
        ...
        "activation_id": V14_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V14_ID}.json",
    ...
    assert sel["activation_id"] == V14_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V14_FROZEN",
        "verdicts": [
            "V1R_TEARDOWN_EXTERNAL_BACKUP_LOGGER_FIXED",
            "V1R_V14_RUNTIME_REGRESSION_PASS",
            "V1R_EXIT_V2_RUNTIME_ACTIVATION_V14_FROZEN",
            "V1R_ACTIVATION_V14_STARTUP_BINDING_VERIFIED",
        ],
        "activation_id": V14_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        ...
        "startup_binding_verdict": "V1R_ACTIVATION_V14_STARTUP_BINDING_VERIFIED",
    ...
    (OUT / "report_activation_v14.json").write_text(
