#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V6.

Parent V5 is immutable history. Does not mutate Strategy / Precommit / V5/V4 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V5"
PARENT_SHA = "885e2ba246ecfdc448274adafaaf1789c7ccb874a3290cc497346cc48aee5e3a"
V6_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V6"
SUPERSEDE_REASON = "RUNTIME_REGISTRATION_OWNERSHIP_AND_DRIFT_RECOVERY_FIX"
...
def main() -> None:
    ...
    v5 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v5["sha256"] == PARENT_SHA == manifest_content_sha(v5), "V5 must remain immutable"
    ...
    activation = {k: v for k, v in v5.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V6_ID,
    ...
    assert activation["guard"] == v5["guard"]
    assert activation["continuation"] == v5["continuation"]
    assert activation["cap"] == v5["cap"]
    assert activation["wait_sec"] == v5["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V6_ID}.json"
    ...
    v5_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v5_after["sha256"] == PARENT_SHA == manifest_content_sha(v5_after)
    ...
    assert sel["activation_id"] == V6_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V6_FROZEN",
        "activation_id": V6_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v6.json").write_text(
