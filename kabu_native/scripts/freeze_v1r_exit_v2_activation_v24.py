#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V24.

Parent V23 is immutable history. Does not mutate Strategy / Precommit / V23–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V23"
PARENT_SHA = "cc116075c66971d438d5f49fa437eb54155d29f20fb2b83d014f6c3e2a66146d"
V24_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V24"
SUPERSEDE_REASON = "RUNTIME_CERT_STAGE_TOKEN_BINDING_AND_DETERMINISTIC_INPUT_DELIVERY_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V24_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V23 must remain immutable"
    ...
    dump_runtime_artifact_inventory(OUT / "V24_RUNTIME_ARTIFACT_DECISION_INVENTORY.json")
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V24 binds "
        "current-stage token identity and deterministic certification delivery. "
        "V23 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V24_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V24_ID}.json",
    ...
    assert sel["activation_id"] == V24_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V24_FROZEN",
        "activation_id": V24_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v23_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v24.json").write_text(
