#!/usr/bin/env python3
"""
Phase 160: momentum_fade / price_momentum_fade exit review (all sessions).

Example::
    python kabu_native/scripts/run_phase160_fade_exit_review.py
"""
...
def main() -> int:
    ...
    from research.phase160_fade_exit_review import analyze_phase160, write_phase160_outputs
    ...
    result = analyze_phase160(
        session_dirs,
        pilot_config=config,
        cap5_csv=cap5_csv if cap5_csv.is_file() else None,
    )
    outputs = write_phase160_outputs(result, reports_dir=reports_dir, docs_dir=docs_dir)

    report = {
        k: v
        for k, v in result.items()
        if k not in ("fade_events", "path_rows", "class_rows", "rule_candidates", "policy_whatif")
    }
    report["generated_at"] = datetime.now(JST).isoformat(timespec="seconds")
    report["session_count"] = len(session_dirs)
    report["outputs"] = outputs
    report["policy_whatif"] = result.get("policy_whatif")
    report["top_breakdown_rules"] = sorted(
    ...
    valid = (
        "fade_exit_too_fast",
        "fade_exit_reasonable",
        "breakdown_rule_promising",
        "need_tick_density",
        "mixed_result",
    )
    return 0 if result["verdict"] in valid else 1
