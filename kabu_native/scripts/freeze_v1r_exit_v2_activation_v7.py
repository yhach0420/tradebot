#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V7.

Parent V6 is immutable history. Does not mutate Strategy / Precommit / V6/V5/V4 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V6"
PARENT_SHA = "f113dd0d77138417a0b32c2666edb99b2d257163a43805dad4a2c032e6d4c86f"
V7_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V7"
SUPERSEDE_REASON = "RUNTIME_PREWARMUP_PREFLIGHT_AND_EXITCODE_PROPAGATION_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V7_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v6 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v6["sha256"] == PARENT_SHA == manifest_content_sha(v6), "V6 must remain immutable"
    ...
    activation = {k: v for k, v in v6.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V7_ID,
    ...
    assert activation["guard"] == v6["guard"]
    assert activation["continuation"] == v6["continuation"]
    assert activation["cap"] == v6["cap"]
    assert activation["wait_sec"] == v6["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V7_ID}.json"
    ...
    v6_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v6_after["sha256"] == PARENT_SHA == manifest_content_sha(v6_after)
    ...
    assert sel["activation_id"] == V7_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V7_FROZEN",
        "activation_id": V7_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v7.json").write_text(
