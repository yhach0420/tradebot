#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V8.

Parent V7 is immutable history. Does not mutate Strategy / Precommit / V7/V6/V5/V4 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V7"
PARENT_SHA = "3074c3bf8028819e2f708550c5cdbff874fb4a7f3e7274e7327a48dcf2fda087"
V8_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V8"
SUPERSEDE_REASON = "RUNTIME_FROZEN_AM_UNIVERSE_SINGLE_AUTHORITY_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V8_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v7 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v7["sha256"] == PARENT_SHA == manifest_content_sha(v7), "V7 must remain immutable"
    ...
    activation = {k: v for k, v in v7.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V8_ID,
    ...
    assert activation["guard"] == v7["guard"]
    assert activation["continuation"] == v7["continuation"]
    assert activation["cap"] == v7["cap"]
    assert activation["wait_sec"] == v7["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V8_ID}.json"
    ...
    v7_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v7_after["sha256"] == PARENT_SHA == manifest_content_sha(v7_after)
    ...
    assert sel["activation_id"] == V8_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V8_FROZEN",
        "activation_id": V8_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v8.json").write_text(
