#!/usr/bin/env python3
"""
Phase 161: fade shadow policy A/B replay (review only).

Example::
    python kabu_native/scripts/run_phase161_fade_shadow_policy_review.py
"""
...
def main() -> int:
    ...
    result = analyze_phase161(
        session_dirs,
        pilot_config=config,
        cap5_csv=cap5_csv if cap5_csv.is_file() else None,
    )
    outputs = write_phase161_outputs(result, reports_dir=reports_dir, docs_dir=docs_dir)
    ...
    report["scenario_rows"] = result.get("scenario_rows")
    report["risk_summary"] = result.get("risk_summary")
    ...
    valid = {
        "two_tick_delay_promising",
        "breakdown_confirmed_promising",
        "take_reached_only_promising",
        "fade_disable_best_but_risky",
        "current_fade_best",
        "mixed_needs_live_shadow",
    }
