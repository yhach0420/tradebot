#!/usr/bin/env python3
"""Phase361: Near day high + low momentum ENTRY guard shadow validation."""
...
def _worker(job: dict) -> dict:
    _bootstrap()
    from small_paper.near_day_high_low_mom_entry_guard_shadow import evaluate_session
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase361 near day high low mom guard validation")
    ...
    from research.phase361_near_day_high_low_mom_guard_validation import (
        Phase361NearDayHighLowMomValidation,
    )
    ...
    audit = Phase361NearDayHighLowMomValidation(reports_dir=REPORTS)
    jobs = [{"session_meta": meta, "reports_dir": str(REPORTS)} for meta in sessions]

    print(
        f"phase361 sessions={len(sessions)} parallel={args.parallel} "
