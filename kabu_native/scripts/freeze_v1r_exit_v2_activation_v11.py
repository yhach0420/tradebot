#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V11.

Parent V10 is immutable history. Does not mutate Strategy / Precommit / V10–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V10"
PARENT_SHA = "84d76713fd8492a8e518b64d2818f724657572588adbb371b6364439ebd7c0b2"
V11_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V11"
SUPERSEDE_REASON = "RUNTIME_KABU_API_SINGLE_AUTHORITY_AND_PM_DIRECT_START_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V11_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v10 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v10["sha256"] == PARENT_SHA == manifest_content_sha(v10), "V10 must remain immutable"
    ...
    activation = {k: v for k, v in v10.items() if k != "sha256"}
    invalid = dict(activation.get("invalid_prospective_days") or {})
    invalid["20260813"] = {
        "status": "OPERATIONAL_DIAGNOSTIC_ONLY / INVALID_FOR_STRATEGY_EVALUATION",
        "reason": (
            "V10 live operational validation failed "
            "(KABU_API_TOKEN_OWNERSHIP_CONFLICT / POST_SESSION_AM_SAFETY_SIDE_EFFECT / "
            "INGRESS_REGISTRATION_RETRY_STORM). V11 is the runtime fix; not Prospective."
        ),
    }
    activation.update(
        {
    ...
    assert activation["guard"] == v10["guard"]
    assert activation["continuation"] == v10["continuation"]
    assert activation["cap"] == v10["cap"]
    assert activation["wait_sec"] == v10["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V11_ID}.json"
    ...
    v10_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v10_after["sha256"] == PARENT_SHA == manifest_content_sha(v10_after)
    ...
    selector = {
        ...
        "activation_id": V11_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V11_ID}.json",
    ...
    assert sel["activation_id"] == V11_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V11_FROZEN",
        "activation_id": V11_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v11.json").write_text(
