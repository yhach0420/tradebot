#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G7 Candidate-7 snapshot from current working tree.

Does not mutate Formal V25 or Candidate-6. Does not Formal-freeze V26.
Does not enable Formal Paper. OPVAL selector is rewritten separately.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G7_7"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g7_7.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-7 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G7_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G7_UNCERTIFIED_CURRENT_RUNTIME_OPVAL_CANDIDATE7",
