#!/usr/bin/env python
"""2026-08-12 Full Capture Replay after pre-warmup residency / exit-code fix.

V6/V5/V4 manifests are immutable history. Strategy/Precommit unchanged.
"""
...
OUT = ROOT / "results" / "research" / "v1r_v7_full_replay_20260812"
...
def v6_immutable_precheck() -> dict:
    ...
    checks = {
        ...
        "strategy": v6.get("strategy_sha") == STRATEGY_SHA == v5.get("strategy_sha") == v4.get("strategy_sha"),
        "precommit": v6.get("precommit_sha") == PRECOMMIT_SHA == v5.get("precommit_sha"),
    ...
    return {
        ...
        "note": "V6/V5/V4 immutable; V7 freeze is separate",
    }
...
def main() -> int:
    ...
    v4h.OUT = OUT
    v4h.v3_immutable_precheck = v6_immutable_precheck  # type: ignore[method-assign]
    ...
    if report_path.is_file():
        ...
        if rep.get("verdict") == "V1R_V4_20260812_FULL_REPLAY_E2E_PASS":
            rep["verdict"] = "V1R_V7_20260812_FULL_REPLAY_E2E_PASS"
        elif rep.get("verdict") == "V1R_V4_20260812_FULL_REPLAY_E2E_FAIL":
            rep["verdict"] = "V1R_V7_20260812_FULL_REPLAY_E2E_FAIL"
        report_path.write_text(json.dumps(rep, indent=2, ensure_ascii=False, default=str) + "\n", encoding="utf-8")
        print("V7_REPLAY_VERDICT", rep.get("verdict"), "blockers", rep.get("blockers"), flush=True)
