#!/usr/bin/env python3
"""Phase592A — Kabu API online verification (no sendorder)."""
...
def main() -> int:
    ...
    from research.phase592a_kabu_api_online_verification import Phase592AJob

    job = Phase592AJob(repo_root=REPO, rtt_samples=samples)
    result = job.run()
    paths = job.write_outputs(result)
    print(f"verdict={result.get('verdict')}", flush=True)
    print(f"preflight_ready={result.get('preflight_ready')}", flush=True)
