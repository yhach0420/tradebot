#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V15.

Parent V14 is immutable history. Does not mutate Strategy / Precommit / V14–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V14"
PARENT_SHA = "36c48927c36957a20ffc9cd8627e2805b6ea3f17cb9685404254aa5a92e950a2"
V15_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V15"
SUPERSEDE_REASON = "RUNTIME_FULL_DAY_PRE_PAPER_CERTIFICATION_GATE"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V15_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v14 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v14["sha256"] == PARENT_SHA == manifest_content_sha(v14), "V14 must remain immutable"
    ...
    activation = {k: v for k, v in v14.items() if k != "sha256"}
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V15 adds the "
        "full-day pre-paper certification gate; V14 is immutable parent history."
    )
    ...
    assert activation["guard"] == v14["guard"]
    assert activation["continuation"] == v14["continuation"]
    assert activation["cap"] == v14["cap"]
    assert activation["wait_sec"] == v14["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V15_ID}.json"
    ...
    v14_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v14_after["sha256"] == PARENT_SHA == manifest_content_sha(v14_after)
    ...
    selector = {
        ...
        "activation_id": V15_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V15_ID}.json",
    ...
    assert sel["activation_id"] == V15_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V15_FROZEN",
        "activation_id": V15_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v14_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v15.json").write_text(
