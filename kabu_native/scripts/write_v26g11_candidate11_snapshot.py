#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G11 Candidate-11 snapshot from current working tree.

Does not mutate Formal V25 or Candidate-6/7/8/9/10. Does not Formal-freeze V26.
Does not enable Formal Paper. OPVAL current-trading-day identity is not rewritten.
Candidate-10 DualLane bytes must remain identical.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G11_11"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g11_11.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-11 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    strategy_ok = (
        str(v25.get("strategy_sha") or "") == str(c10.get("strategy_sha") or "") == str(c6.get("strategy_sha") or "")
    ...
    if not strategy_ok:
        print("REFUSE: strategy identity drift vs Candidate-6/10 pins")
        return 2

    cov = audit_runtime_inventory_coverage(native_root=NATIVE)
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G11_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    if str(inv.get(dual_rel) or "") != C10_DUALLANE_SHA:
        ...
        return 2
    c10_inv = c10.get("runtime_file_sha256") or {}
    drifted = [
        rel
        for rel in RUNTIME_DEPENDENCY_RELS
        if rel not in ALLOWED_RUNTIME_DRIFT and str(c10_inv.get(rel) or "") != str(inv.get(rel) or "")
    ]
    if drifted:
        print("REFUSE: unexpected inventory drift vs Candidate-10 G=", len(drifted), drifted[:20])
        return 2
    runtime_changed = [
        rel
        for rel in sorted(set(ALLOWED_RUNTIME_DRIFT) | set(RUNTIME_DEPENDENCY_RELS))
        if rel in ALLOWED_RUNTIME_DRIFT and str(c10_inv.get(rel) or "") != str(inv.get(rel) or "")
    ]
    if "src/small_paper/local_market_bus.py" not in runtime_changed:
        print("REFUSE: expected local_market_bus fanout repair vs Candidate-10")
        return 2
    if "src/small_paper/paper_market_bus_consumer.py" not in runtime_changed:
        print("REFUSE: expected paper_market_bus_consumer resync repair vs Candidate-10")
        return 2
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(c10.get(key) or "") != str(c7.get(key) or "") or str(c10.get(key) or "") != str(v25.get(key) or ""):
            print("REFUSE: Candidate-10 strategy identity drifted vs V25/C7", key)
            return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G11_ATOMIC_MIDDAY_REALTIME_RESYNC_REPAIR",
            ...
            "parent_runtime_candidate_id": C10_ID,
            "parent_runtime_candidate_sha": C10_SHA,
            "parent_runtime_candidate_status": "UNCERTIFIED_IMMUTABLE_LEFT_IN_PLACE",
            "allowed_runtime_drift_vs_candidate10": sorted(ALLOWED_RUNTIME_DRIFT),
    ...
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(body.get(key) or "") != str(c10.get(key) or ""):
            print("REFUSE: strategy SHA != Candidate-10", key, body.get(key), c10.get(key))
            return 2
    ...
    selector = {
        ...
        "note": (
            "Identity-only UNCERTIFIED candidate-11 selector; not the active Formal selector. "
            "Not Candidate-10. OPVAL current-trading-day identity is not rewritten."
        ),
    }
