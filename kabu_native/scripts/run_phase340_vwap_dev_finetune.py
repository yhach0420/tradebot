#!/usr/bin/env python3
"""
Phase340: VWAP deviation fine-tuning around 0.5%.

Default: --max-sessions 3 --max-rows-per-session 150000 --streaming
Output: kabu_native/results/reports/phase340_vwap_dev_finetune_*
"""
...
def _run_session(
    *,
    ...
    out_dir = resolve_output_dir(cfg, repo_root=REPO, day_key=day_key) / f"phase340_{stamp}"
    ...
    try:
        result = run_push_replay_dry_run(
            ...
            vwap_tuning_variants=list(variants),
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Phase340 VWAP dev fine-tuning")
    ...
    from research.phase340_vwap_dev_finetune_evaluation import Phase340IncrementalAggregator
    ...
    agg = Phase340IncrementalAggregator(reports_dir=REPORTS, variants=variants)
    ...
    print(
        f"phase340 sessions={len(sessions)} variants={len(variants)} "
    ...
    print("\n=== Phase340 Summary ===", flush=True)
    print(f"actual_pnl: {summary.get('actual_total_pnl_yen_100')} pf: {summary.get('actual_pf')}", flush=True)
    print(f"best_tradeoff: {summary.get('best_variant_by_tradeoff')}", flush=True)
    print(f"adopt_ready: {summary.get('adopt_ready_variants')}", flush=True)
    for row in (summary.get("tradeoff_comparison") or []):
        print(
            f"  {row.get('variant_id')}: tradeoff={row.get('tradeoff_score')} "
            f"stop_red={row.get('stop_hit_reduction_count')} miss={row.get('profit_take_miss_yen_100')} "
            f"sessions+{row.get('improved_session_count')}/-{row.get('worsened_session_count')}",
            flush=True,
        )
