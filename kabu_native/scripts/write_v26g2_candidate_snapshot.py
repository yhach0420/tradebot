#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G2 Candidate-1 snapshot.

Does not mutate V25 selector/manifest. Does not Formal-freeze V26.
Refuses overwrite if the candidate files already exist.
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G2_1"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g2_1.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G2_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2

    inv = collect_runtime_inventory(native_root=NATIVE)
    if len(inv) != len(RUNTIME_DEPENDENCY_RELS):
        print("REFUSE: inventory length != generator")
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G2_UNCERTIFIED_PRE_CERTIFICATION_CANDIDATE_SNAPSHOT",
    ...
    selector = {
        ...
        "note": "Identity-only UNCERTIFIED candidate selector; not the active Formal selector.",
    }
