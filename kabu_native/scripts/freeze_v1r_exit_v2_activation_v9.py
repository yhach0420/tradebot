#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V9.

Parent V8 is immutable history. Does not mutate Strategy / Precommit / V8/V7 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V8"
PARENT_SHA = "1d3e7c3d9644b7db60f4cc2524ba2ce4065f78fe4cff3cec86971ff97e25954f"
V9_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V9"
SUPERSEDE_REASON = "RUNTIME_REGISTERED_KABU_PROBE_SINGLE_AUTHORITY_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V9_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    v8 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v8["sha256"] == PARENT_SHA == manifest_content_sha(v8), "V8 must remain immutable"
    ...
    activation = {k: v for k, v in v8.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V9_ID,
    ...
    assert activation["guard"] == v8["guard"]
    assert activation["continuation"] == v8["continuation"]
    assert activation["cap"] == v8["cap"]
    assert activation["wait_sec"] == v8["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V9_ID}.json"
    ...
    v8_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v8_after["sha256"] == PARENT_SHA == manifest_content_sha(v8_after)
    ...
    assert sel["activation_id"] == V9_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V9_FROZEN",
        "activation_id": V9_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v9.json").write_text(
