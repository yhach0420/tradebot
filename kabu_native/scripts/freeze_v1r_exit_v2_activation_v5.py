#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V5.

Parent V4 is immutable history. Does not mutate Strategy / Precommit / V4 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V4"
PARENT_SHA = "73e9397ba7e2cee05f32044c3cb5ecb80459f45d191fdb50496a1c95f4a86dc2"
V5_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V5"
SUPERSEDE_REASON = "RUNTIME_SAME_DAY_AM_INGRESS_REGISTRATION_BINDING_FIX"
...
def main() -> None:
    ...
    v4 = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v4["sha256"] == PARENT_SHA == manifest_content_sha(v4), "V4 must remain immutable"
    ...
    activation = {k: v for k, v in v4.items() if k != "sha256"}
    activation.update(
        {
            "manifest_id": V5_ID,
    ...
    assert activation["guard"] == v4["guard"]
    assert activation["continuation"] == v4["continuation"]
    assert activation["cap"] == v4["cap"]
    assert activation["wait_sec"] == v4["wait_sec"]

    activation["sha256"] = manifest_content_sha(activation)
    path = OUT / f"{V5_ID}.json"
    ...
    v4_after = json.loads((OUT / f"{PARENT_ID}.json").read_text(encoding="utf-8"))
    assert v4_after["sha256"] == PARENT_SHA == manifest_content_sha(v4_after)
    ...
    assert sel["activation_id"] == V5_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V5_FROZEN",
        "activation_id": V5_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
    ...
    (OUT / "report_activation_v5.json").write_text(
