#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V20.

Parent V19 is immutable history. Does not mutate Strategy / Precommit / V19–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V19"
PARENT_SHA = "cb0778016b67bf50e75d95973f13dad10fc9cfc3f04995a39df0bb66a425357e"
V20_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V20"
SUPERSEDE_REASON = "RUNTIME_STALE_SHARED_TOKEN_PRE_INGRESS_REUSE_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V20_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V19 must remain immutable"
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V20 refuses leftover "
        "day-dir tokens unless a live Ingress owner holds Station authority. "
        "V19 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V20_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V20_ID}.json",
    ...
    assert sel["activation_id"] == V20_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V20_FROZEN",
        "activation_id": V20_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v19_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v20.json").write_text(
