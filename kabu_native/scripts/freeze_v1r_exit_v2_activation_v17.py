#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V17.

Parent V16 is immutable history. Does not mutate Strategy / Precommit / V16–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V16"
PARENT_SHA = "3d9a393bb691d6ffa9ac6746fb720ce6f7644d6dcd9de2915d88bef99db67892"
V17_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V17"
SUPERSEDE_REASON = "RUNTIME_FULL_DAY_CERT_SESSION_CLOCK_ARM_AT_LAUNCHER"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V17_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V17 arms the "
        "session clock at launcher start so checked-runner preflight cannot "
        "consume virtual AM. V16 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V17_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V17_ID}.json",
    ...
    assert sel["activation_id"] == V17_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V17_FROZEN",
        "activation_id": V17_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v16_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v17.json").write_text(
