#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V25.

Parent V24 is immutable history. Does not mutate Strategy / Precommit / V24–V1 bytes.
...
from small_paper.derived_artifact_contract import dump_runtime_artifact_inventory
...
V20_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V20"
V20_SHA = "0bf8e9b2237d7fe281b2dfa735e4a7b709c6da5c85da9eafa5e43b662569dc14"
V21_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V21"
V21_SHA = "bb5c2ab9e47395f8d72bee6a68a2a42308d38a19df7810cd47f719a098cac52c"
V22_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V22"
V22_SHA = "6c2d05dde6f64b4838a6b69c009c34981bf054ad4a253d85aa3aefdd05f29d34"
V23_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V23"
V23_SHA = "cc116075c66971d438d5f49fa437eb54155d29f20fb2b83d014f6c3e2a66146d"
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V24"
PARENT_SHA = "bf06d055bffb790533e116655bd3c9a3b0887fd0289a07a0247603e6368861ae"
V25_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V25"
SUPERSEDE_REASON = "RUNTIME_AUTH_LIFECYCLE_PHASE_POLICY_AND_PRE_INGRESS_STARTUP_DEADLOCK_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V25_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V24 must remain immutable"
    v23 = json.loads((OUT / f"{V23_ID}.json").read_text(encoding="utf-8"))
    assert v23["sha256"] == V23_SHA == manifest_content_sha(v23), "V23 must remain immutable"
    v22 = json.loads((OUT / f"{V22_ID}.json").read_text(encoding="utf-8"))
    assert v22["sha256"] == V22_SHA == manifest_content_sha(v22), "V22 must remain immutable"
    v21 = json.loads((OUT / f"{V21_ID}.json").read_text(encoding="utf-8"))
    assert v21["sha256"] == V21_SHA == manifest_content_sha(v21), "V21 must remain immutable"
    v20 = json.loads((OUT / f"{V20_ID}.json").read_text(encoding="utf-8"))
    assert v20["sha256"] == V20_SHA == manifest_content_sha(v20), "V20 must remain immutable"
    ...
    dump_runtime_artifact_inventory(OUT / "V25_RUNTIME_ARTIFACT_DECISION_INVENTORY.json")
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V25 binds "
        "lifecycle-phase AUTH (PRE_INGRESS defer vs POST_INGRESS fail-close) "
        "and leftover process/token cleanup. V24 is immutable parent."
    )
    ...
    v23_after = json.loads((OUT / f"{V23_ID}.json").read_text(encoding="utf-8"))
    assert v23_after["sha256"] == V23_SHA == manifest_content_sha(v23_after)
    v22_after = json.loads((OUT / f"{V22_ID}.json").read_text(encoding="utf-8"))
    assert v22_after["sha256"] == V22_SHA == manifest_content_sha(v22_after)
    v21_after = json.loads((OUT / f"{V21_ID}.json").read_text(encoding="utf-8"))
    assert v21_after["sha256"] == V21_SHA == manifest_content_sha(v21_after)
    v20_after = json.loads((OUT / f"{V20_ID}.json").read_text(encoding="utf-8"))
    assert v20_after["sha256"] == V20_SHA == manifest_content_sha(v20_after)
    ...
    selector = {
        ...
        "activation_id": V25_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V25_ID}.json",
    ...
    assert sel["activation_id"] == V25_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V25_FROZEN",
        "activation_id": V25_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v20_immutable": V20_SHA,
        "v21_immutable": V21_SHA,
        "v22_immutable": V22_SHA,
        "v23_immutable": V23_SHA,
        "v24_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v25.json").write_text(
