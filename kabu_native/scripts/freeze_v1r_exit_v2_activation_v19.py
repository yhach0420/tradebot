#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V19.

Parent V18 is immutable history. Does not mutate Strategy / Precommit / V18–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V18"
PARENT_SHA = "bb301468b9a468589fee530cef6a3df027e8ed20e21e2f82fc955caa9b485882"
V19_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V19"
SUPERSEDE_REASON = "RUNTIME_GLOBAL_KABU_TOKEN_SINGLE_AUTHORITY_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V19_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V18 must remain immutable"
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V19 binds a "
        "Station-global single Kabu token authority so a second process cannot "
        "POST /token. V18 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V19_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V19_ID}.json",
    ...
    assert sel["activation_id"] == V19_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V19_FROZEN",
        "activation_id": V19_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v18_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v19.json").write_text(
