#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G3 Candidate-2 snapshot.

Does not mutate V25 selector/manifest.
Does not rewrite Candidate-1 (FAILED / AUDIT_ONLY).
Does not Formal-freeze V26.
Refuses overwrite if Candidate-2 files already exist.
Must run only after production source is frozen for this candidate.
"""
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G3_2"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g3_2.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-2 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G3_UNCERTIFIED_RUNTIME_FIX_CANDIDATE2_SNAPSHOT",
    ...
    selector = {
        ...
        "note": "Identity-only UNCERTIFIED candidate-2 selector; not the active Formal selector.",
    }
    ...
    if c1_after.get("sha256") != CANDIDATE1_SHA:
        print("REFUSE: candidate-1 mutated during candidate-2 write")
        return 2
