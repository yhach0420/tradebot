#!/usr/bin/env python
"""Write UNCERTIFIED immutable V26-G9 Candidate-9 snapshot from current working tree.

Does not mutate Formal V25, Candidate-6, Candidate-7, or Candidate-8. Does not Formal-freeze V26.
...
CANDIDATE_ID = "V1R_EXIT_V2_PAPER_PRIMARY_CANDIDATE_V26G9_9"
SELECTOR_CANDIDATE = OUT / "active_v1r_candidate_v26g9_9.json"
...
def main() -> int:
    ...
    if dest.is_file() or SELECTOR_CANDIDATE.is_file():
        print("REFUSE_OVERWRITE: candidate-9 snapshot already exists", dest, SELECTOR_CANDIDATE)
        return 2
    ...
    if prior_shas[C8_ID] != C8_SHA or not C8_SHA:
        print("REFUSE: Candidate-8 manifest mutated or missing sha")
        return 2
    ...
    if not cov.get("ok") or cov.get("runtime_critical_uncovered_files"):
        print("V1R_V26G9_INVENTORY_COVERAGE_FAIL", json.dumps(cov, indent=2, default=str))
        return 2
    ...
    if len(inv) != len(RUNTIME_DEPENDENCY_RELS):
        ...
        return 2
    c8_inv = c8.get("runtime_file_sha256") or {}
    drifted = [
        rel
        for rel in RUNTIME_DEPENDENCY_RELS
        if rel not in NOTIFY_ONLY_RELS and str(c8_inv.get(rel) or "") != str(inv.get(rel) or "")
    ]
    if drifted:
        print("REFUSE: strategy-affecting inventory drift G=", len(drifted), drifted[:20])
        return 2
    notify_changed = [rel for rel in sorted(NOTIFY_ONLY_RELS) if str(c8_inv.get(rel) or "") != str(inv.get(rel) or "")]
    if not notify_changed:
        print("REFUSE: expected notification-only inventory drift vs Candidate-8")
        return 2
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(c8.get(key) or "") != str(c7.get(key) or "") or str(c8.get(key) or "") != str(v25.get(key) or ""):
            print("REFUSE: Candidate-8 strategy identity drifted vs V25/C7", key)
            return 2
    ...
    body.update(
        {
            ...
            "supersede_reason": "V26G9_PBV2_DISCORD_NOTIFICATION_ONLY",
            "notification_only_change": True,
            ...
            "parent_runtime_candidate_id": C8_ID,
            "parent_runtime_candidate_sha": C8_SHA,
            "parent_runtime_candidate_status": "UNCERTIFIED_IMMUTABLE_LEFT_IN_PLACE",
        }
    )
    for key in ("entry_sha", "anchor_sha", "exit_v2_candidate_sha", "strategy_sha"):
        if str(body.get(key) or "") != str(c8.get(key) or ""):
            print("REFUSE: strategy SHA != Candidate-8", key, body.get(key), c8.get(key))
            return 2
    if str(body.get("submit_cancel_live") or "") not in {"0/0/0", "0/0/0"}:
        # keep explicit 0/0/0
        pass
    ...
    selector = {
        ...
        "note": (
            "Identity-only UNCERTIFIED candidate-9 selector; not the active Formal selector. "
            "Not Candidate-8. OPVAL current-trading-day identity is not rewritten."
        ),
    }
    ...
    c8_after = json.loads((OUT / f"{C8_ID}.json").read_text(encoding="utf-8"))
    ...
    if c7_after.get("sha256") != C7_SHA:
        ...
        SELECTOR_CANDIDATE.unlink(missing_ok=True)
        return 2
    if c8_after.get("sha256") != C8_SHA:
        print("REFUSE: Candidate-8 mutated during candidate write")
        dest.unlink(missing_ok=True)
        SELECTOR_CANDIDATE.unlink(missing_ok=True)
        return 2
    ...
    print("CANDIDATE8_UNCHANGED=true")
    print("RUNTIME_CHANGED=true")
    print("NOTIFICATION_ONLY_CHANGE=true")
