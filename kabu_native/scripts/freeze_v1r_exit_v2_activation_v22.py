#!/usr/bin/env python
"""Create V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V22.

Parent V21 is immutable history. Does not mutate Strategy / Precommit / V21–V1 bytes.
...
PARENT_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V21"
PARENT_SHA = "bb5c2ab9e47395f8d72bee6a68a2a42308d38a19df7810cd47f719a098cac52c"
V22_ID = "V1R_EXIT_V2_PAPER_PRIMARY_ACTIVATION_V22"
SUPERSEDE_REASON = "RUNTIME_CURRENT_RUN_ARTIFACT_SOT_AND_CERT_SCOPE_FIX"
...
def _assert_inventory_committed() -> None:
    ...
    if dirty:
        raise SystemExit(
            "REFUSE_V22_FREEZE: runtime inventory uncommitted: " + ", ".join(dirty)
        )
...
def main() -> None:
    ...
    assert parent["sha256"] == PARENT_SHA == manifest_content_sha(parent), "V21 must remain immutable"
    ...
    dump_runtime_artifact_inventory(OUT / "V22_RUNTIME_ARTIFACT_DECISION_INVENTORY.json")
    ...
    day14["note"] = (
        ...
        "INVALID_FOR_STRATEGY_EVALUATION / NOT_PROSPECTIVE_DAY1. V22 re-evaluates "
        "current-run derived artifacts and isolates certification dest by "
        "certification_run_id/stage_run_id. V21 is immutable parent."
    )
    ...
    selector = {
        ...
        "activation_id": V22_ID,
        "activation_sha": activation["sha256"],
        "manifest_relpath": f"{V22_ID}.json",
    ...
    assert sel["activation_id"] == V22_ID
    assert sel["activation_sha"] == activation["sha256"]

    report = {
        "verdict": "V1R_EXIT_V2_RUNTIME_ACTIVATION_V22_FROZEN",
        "activation_id": V22_ID,
        "activation_sha": activation["sha256"],
        "parent_activation_id": PARENT_ID,
        "parent_activation_sha": PARENT_SHA,
        ...
        "v21_immutable": PARENT_SHA,
        "day_20260814": ["INVALID_FOR_STRATEGY_EVALUATION", "NOT_PROSPECTIVE_DAY1"],
        "paper_started": False,
    }
    (OUT / "report_activation_v22.json").write_text(
