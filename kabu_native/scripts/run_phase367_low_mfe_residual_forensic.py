...
def _worker(job: dict) -> dict:
    _bootstrap()
    from research.phase367_low_mfe_residual_forensic import load_session_residual_forensic

    t0 = time.monotonic()
    try:
        result = load_session_residual_forensic(
            job["session_meta"],
            reports_dir=Path(job["reports_dir"]),
        )
        if int(result.get("residual_count") or 0) == 0 and not (
            result.get("all_production_enriched") or []
        ):
            return {
                "ok": False,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Phase367 low-MFE residual forensic")
    ...
    from research.phase367_low_mfe_residual_forensic import (
        MIN_DAY,
        Phase367LowMfeResidualForensic,
    )
    ...
    audit = Phase367LowMfeResidualForensic(reports_dir=REPORTS)
    jobs = [{"session_meta": meta, "reports_dir": str(REPORTS)} for meta in sessions]

    print(
        f"phase367 sessions={len(sessions)} parallel={args.parallel} "
        ...
        with ProcessPoolExecutor(max_workers=max(1, args.max_workers)) as pool:
            ...
            for fut in as_completed(futures):
                ...
                if st.get("ok"):
                    ...
                    print(f"  ok {st.get('session_id')} residual={n}", flush=True)
                else:
                    errors += 1
    ...
    print("\n=== Phase367 Summary ===", flush=True)
    print(f"residual_count: {summary.get('residual_stop_count')}", flush=True)
    print(f"total_loss: {summary.get('total_residual_loss_yen_100')}", flush=True)
    print(f"largest_pattern: {conc.get('largest_residual_loss_pattern')}", flush=True)
    print(f"best_cf_pattern: {conc.get('best_counterfactual_pattern')}", flush=True)
    print(f"expected_improvement: {conc.get('expected_improvement_yen_100')}", flush=True)
    print(f"adoptable_candidate: {conc.get('adoptable_guard_candidate')}", flush=True)
