...
class TestPhase237EntryExpectancyScoreV2Shadow(unittest.TestCase):
    ...
    def test_v2_excludes_rollingmae_mid_only(self) -> None:
        self.assertNotIn("RollingMAE:mid", SCORE_POINTS_V2)
        self.assertIn("HBRecent:no", SCORE_POINTS_V2)
        self.assertEqual(SCORE_POINTS_V2, {k: v for k, v in SCORE_POINTS.items() if k != "RollingMAE:mid"})

    def test_v1_matches_v2_when_rollingmae_mid_active(self) -> None:
        trade = {
            "trading_value": 3e10,
        ...
        self.assertEqual(fields["entry_expectancy_score_v2"], 8)
        self.assertTrue(fields["entry_expectancy_score_ge6_flag"])
        self.assertTrue(fields["entry_expectancy_score_v2_ge6_flag"])

    def test_v2_matches_v1_when_rollingmae_not_mid(self) -> None:
        trade = {
            "trading_value": 3e10,
        ...
        fields = compute_entry_expectancy_score_fields(trade=trade)
        self.assertEqual(fields["entry_expectancy_score"], fields["entry_expectancy_score_v2"])
        self.assertEqual(
            fields["entry_expectancy_score_ge5_flag"],
            fields["entry_expectancy_score_v2_ge5_flag"],
        )
    ...
    def test_exit_enrich_and_counters(self) -> None:
        ...
        self.assertEqual(summary["score5_v2_count"], 1)
        self.assertEqual(summary["score6_v2_count"], 1)
