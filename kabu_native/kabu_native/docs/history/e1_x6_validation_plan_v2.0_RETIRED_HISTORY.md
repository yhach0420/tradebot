...
| 制定日 | 2026-07-30 |
| 最終監査日 | 2026-08-02 |
| 状態 | `APPROVED_PLAN / JOINT_STRATEGY_VALIDATION` |
| 状態基準時点 | 2026-08-02。Version 1.3 Stage-1監査完了後。評価単位は`JointStrategyPackage`（ENTRY＋EXIT一体）。Shadowはユーザー承認後のみ。 |
...
### 3.3 Joint FixedSpec 必須ゲート（20260721～20260731）

- 9/9日で5bps控除後PnL > 0（取引ゼロ日はプラス扱いにしない）
- 全9日へ同一ENTRY＋EXIT spec
- `worst_day_net_pnl > 0`
- 各日 completed trades >= 3、全期間 >= 30
- 7/22除外後も PnL > 0 かつ PF > 1.00
- Rolling-origin F1～F5 confirm 5/5プラス
- RefitLODO held-out 9/9プラス
- ENTRY/EXIT family・特徴量方向のfold間反転なし
- 最大利益日寄与率 <= 30%
- top1取引・top1銘柄除外後も全期間PnL > 0
- PF >= 1.10
- E1_X5比で最大DDとSTOP損失が悪化しない
- INVALID_SOURCE混入0、A/B完全一致、report/xlsx独立再集計一致

合格なし → `E1_X6_NO_ROBUST_JOINT_STRATEGY`（条件緩和禁止）。
全ゲート合格でも CORE_VALID=0 なら `E1_X6_JOINT_RESEARCH_SPEC_FROZEN_FOR_FORWARD_TEST`（採用済みと表現しない）。

Shadow有効化、Paper runner/Task/production YAML組込み、20営業日カウント開始、Discord、Runtime反映、Forward開始は**ユーザー明示承認まで禁止**。
